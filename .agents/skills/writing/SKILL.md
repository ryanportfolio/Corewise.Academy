---
name: writing
description: "Use for text that leaves the session (READMEs, docs, site and UI copy, emails, release notes, application answers) or to unslop, humanize, voice-match, or audit a draft for AI tells. Use normal prose for deliverables."
---
...
Out of scope: session replies (Caveman governs those); commits and PR text (follow Codex instructions and repository Git conventions in normal prose); code, identifiers, error strings, quoted text (never reworded); research, legal, technical-reference, and book-chapter work when a specialized skill exists.

## Codex scope and precedence

This is the repository's standalone Codex writing skill. Load it directly from `.agents/skills/writing/SKILL.md`; it does not load a Claude workflow. Resolve `patterns.md` and `NOTICE.md` beside this file. Writing or reviewing does not authorize publishing, Git shipping, or sending messages.

Follow system, developer, and explicit user instructions first. Preserve code, identifiers, errors, and exact quotations. Style edits must never silently alter quoted evidence. For site publication, follow the copy rules' disclosed adaptation procedure; never call an edited quotation word-for-word.

For site copy and guides, read `.claude/reference/voice.md` and `.claude/reference/copy-rules.md` from the repository root. These shared editorial references govern the site's voice and hard gates; other Claude workflow instructions do not apply. Guide titles have a 41-character cap and section headings a 32-character cap. Apply the required layout pass. Recheck the references for current rules when writing.

Within those constraints, preserve the writer's supplied voice: sentence length, register, punctuation, and recurring phrases. Never upgrade casual to corporate. For other deliverables, use the writer's sample before these default rules. The em dash ban applies to newly authored prose, including voice-matched prose, unless the user explicitly overrides it.
...
- **Em dashes (U+2014) in authored prose.** No en-dash or double-hyphen stand-ins. Period, comma, colon, semicolon, parentheses; middle dot (U+00B7) for label separators.
...
1. Em dashes and stand-ins in authored prose: replace every one. Preserve exact quoted evidence and code as described above.
...
For an editorial review, report the verdict separately from the deliverable: **PASS** (no applicable requirement remains) or **FAIL** (each violation named with its rule). Include substance and style in an overall verdict; missing required evidence prevents PASS. Label every remaining note **Requirement** or **Suggestion**; an open requirement means FAIL. Label style-only verdicts as such. Run the sweep silently for drafting tasks; keep review labels and working notes out of finished copy.
