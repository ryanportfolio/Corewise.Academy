...
export const TRACK_NOTES: Record<string, TrackNotes> = {
  ...
  prompting: {
    ...
    notes: [
      'Prompting is the craft of briefing a model, and context is the material you brief it with. This track treats the two as one discipline: what you say, what you attach, and how you arrange both so the model attends to what matters instead of guessing.',
      'The prompting guides run from the fundamentals that hold across every current model to the context budgets that decide what an agent session costs. Reading the whole track takes 26 minutes; the habits it replaces waste far more than that every week.',
    ],
    quotes: [
      {
  ...
  building: {
    ...
    notes: [
      'Building with AI means shipping features other people rely on, which is a different job from getting a demo to work once. This track covers the parts that survive contact with users: API discipline, retrieval, evals, and design decisions that a page can actually defend.',
  ...
  practice: {
    ...
    notes: [
      'Practice is the track for what you do every day: the habits, verification rituals, and taste that decide whether working with AI compounds or corrodes. Knowing when not to use the model is part of the same practice as knowing how.',
